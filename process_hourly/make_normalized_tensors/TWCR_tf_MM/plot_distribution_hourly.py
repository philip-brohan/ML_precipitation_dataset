...
from process_hourly.make_normalized_tensors.TWCR_tf_MM.tensor_utils import (
    tensor_to_cube,
)
...
parser.add_argument(
    "--variable",
    help="Name of variable to use (PRMSL, TMP2m, ...)",
    type=str,
    default="PRATE",
)
...
AvailableHours = raw_zarr.attrs["AvailableHours"]


idx = AvailableHours[
    "%04d-%02d-%02d:%02d_%02d"
    % (args.year, args.month, args.day, args.hour, args.member_idx)
]
raw = tensor_to_cube(
    tf.convert_to_tensor(raw_zarr[:, :, args.member_idx, idx], tf.float32)
)
