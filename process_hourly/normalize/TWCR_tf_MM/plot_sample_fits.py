...
parser.add_argument(
    "--day", help="Day to extract", type=int, required=False, default=12
)
parser.add_argument(
    "--hour", help="Hour to extract", type=int, required=False, default=6
)
parser.add_argument(
    "--variable", help="Variable to use", type=str, required=False, default="PRATE"
)
parser.add_argument(
    "--startyear", help="Start Year", type=int, required=False, default=1969
)
parser.add_argument(
    "--endyear", help="End Year", type=int, required=False, default=1990
)
parser.add_argument(
    "--member_idx",
...
random_i = rng.choice(range(256), size=25, replace=False)
random_j = rng.choice(range(512), size=25, replace=False)
raw = []
for i in range(25):
    raw.append([])
...
(shape, location, scale) = load_fitted(
    args.month, args.day, args.hour, variable=args.variable
)
