...
# Only upload selected ensemble members - saves time and money
...
def upload_file(file_name, file_system_client):
    ...
    if os.path.isdir(file_name):
        ...
        # Only upload .nc files
        print(file_name)
        if not file_name.endswith(".nc"):
            return
        # Only upload selected ensemble members
        member = int(file_name.split("/")[-1].split(".")[-2][-3:])
