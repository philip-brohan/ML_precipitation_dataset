...
from make_normalized_tensors.HadISST_tf_MM.tensor_utils import tensor_to_cube
...
# Get the raw data
raw_zarr = zarr.open(
    "%s/MLP/raw_datasets/HadISST/temperature_zarr" % (os.getenv("SCRATCH"),),
    mode="r",
)
...
normalized_zarr = zarr.open(
    "%s/MLP/normalized_datasets/HadISST_tf_MM/temperature_zarr"
    % (os.getenv("SCRATCH"),),
    mode="r",
)
