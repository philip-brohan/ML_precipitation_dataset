...
fn = "%s/MLP/normalized_datasets/GPCP_tf_MM/precipitation_zarr" % (
    os.getenv("SCRATCH"),
)
...
# Get the raw data
raw_zarr = zarr.open(
    "%s/MLP/raw_datasets/GPCP/blended/precipitation_zarr" % (os.getenv("SCRATCH"),),
    mode="r",
)
...
normalized_zarr = zarr.open(
    "%s/MLP/normalized_datasets/GPCP_tf_MM/precipitation_zarr"
    % (os.getenv("SCRATCH"),),
    mode="r",
)
