...
from make_normalized_tensors.CRU_tf_MM.tensor_utils import tensor_to_cube
...
# Get the raw data
raw_zarr = zarr.open(
    "%s/MLP/raw_datasets/CRU/in_situ/precipitation_zarr" % (os.getenv("SCRATCH"),),
    mode="r",
)
...
normalized_zarr = zarr.open(
    "%s/MLP/normalized_datasets/CRU_tf_MM/precipitation_zarr" % (os.getenv("SCRATCH"),),
    mode="r",
)
