# Create normalized TWCR dataset for analysis
...
def getFileNames(variable, startyear=1850, endyear=2050):
    inFiles = sorted(
        os.listdir(
            "%s/MLP/normalized_datasets/TWCR_tf_MM/%s" % (os.getenv("PDIR"), variable)
        )
    )
    inFiles = [
...
def getDataset(variable, startyear=1850, endyear=2050, blur=None, cache=False):
    ...
    fnFiles = [
        "%s/MLP/normalized_datasets/TWCR_tf_MM/%s/%s" % (os.getenv("PDIR"), variable, x)
        for x in inFiles
    ]
