# Create normalized HadISST dataset for analysis
...
def getFileNames(startyear=1850, endyear=2050):
    inFiles = sorted(
        os.listdir("%s/MLP/normalized_datasets/HadISST_tf_MM/v1" % os.getenv("PDIR"))
    )
    inFiles = [
...
def getDataset(startyear=1850, endyear=2050, blur=None, cache=False):
    ...
    fnFiles = [
        "%s/MLP/normalized_datasets/HadISST_tf_MM/v1/%s" % (os.getenv("PDIR"), x)
        for x in inFiles
    ]
