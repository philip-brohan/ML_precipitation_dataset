...
parser.add_argument("--startyear", type=int, required=False, default=1870)
...
opdir = "%s/MLP/normalized_datasets/HadISST_tf_MM/videos/v1" % (os.getenv("PDIR"),)
...
for year in range(args.startyear, args.endyear + 1):
    for month in range(1, 13):
        if is_done(year, month):
            continue
        cmd = ("./frame.py --year=%d --month=%d --intermediate=%d ") % (
            year,
            month,
            args.intermediate,
        )
