...
def load_tensor(file_name):
    ...
    imt = tf.io.parse_tensor(sict, tf.float32)
...
def getFileNames(startyear=1850, endyear=2050):
    inFiles = sorted(
        os.listdir(
            "%s/MLP/normalised_datasets/HadCRUT_tf_MM/T" % (os.getenv("SCRATCH"))
        )
    )
    inFiles = [
...
def getDataset(startyear=1950, endyear=2014, blur=None, cache=False):
    ...
    fnFiles = [
        "%s/MLP/normalised_datasets/HadCRUT_tf_MM/T/%s" % (os.getenv("SCRATCH"), x)
        for x in inFiles
    ]
