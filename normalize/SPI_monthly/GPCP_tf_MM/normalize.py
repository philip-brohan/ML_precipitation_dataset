...
def load_fitted(month):
    shape = iris.load_cube(
        "%s/normalization/SPI_monthly/GPCP_tf_MM/blended/shape_m%02d.nc"
        % (os.getenv("PDIR"), month),
    )
    location = iris.load_cube(
        "%s/normalization/SPI_monthly/GPCP_tf_MM/blended/location_m%02d.nc"
        % (os.getenv("PDIR"), month),
    )
    scale = iris.load_cube(
        "%s/normalization/SPI_monthly/GPCP_tf_MM/blended/scale_m%02d.nc"
        % (os.getenv("PDIR"), month),
    )
...
def match_original(normalized, gamma_p, norm_mean=0.5025, norm_sd=0.2):
...
def normalize_cube(raw, shape, location, scale, norm_mean=0.5025, norm_sd=0.2):
...
def unnormalize_cube(normalized, shape, location, scale, norm_mean=0.5025, norm_sd=0.2):
