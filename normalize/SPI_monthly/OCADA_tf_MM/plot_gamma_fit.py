...
parser.add_argument(
    "--variable",
    help="Name of variable to use (precipi, ta, psl, ...)",
    type=str,
    default="precipi",
)
...
shape = iris.load_cube(
    "%s/MLP/normalization/SPI_monthly/OCADA_tf_MM/%s/shape_m%02d.nc"
    % (os.getenv("SCRATCH"), args.variable, args.month),
)
location = iris.load_cube(
    "%s/MLP/normalization/SPI_monthly/OCADA_tf_MM/%s/location_m%02d.nc"
    % (os.getenv("SCRATCH"), args.variable, args.month),
)
scale = iris.load_cube(
    "%s/MLP/normalization/SPI_monthly/OCADA_tf_MM/%s/scale_m%02d.nc"
    % (os.getenv("SCRATCH"), args.variable, args.month),
)
