...
def load_fitted(month):
    shape = iris.load_cube(
        "%s/MLP/normalisation/SPI_monthly/GPCC_tf_MM/blended/shape_m%02d.nc"
        % (os.getenv("SCRATCH"), month),
    )
    location = iris.load_cube(
        "%s/MLP/normalisation/SPI_monthly/GPCC_tf_MM/blended/location_m%02d.nc"
        % (os.getenv("SCRATCH"), month),
    )
    scale = iris.load_cube(
        "%s/MLP/normalisation/SPI_monthly/GPCC_tf_MM/blended/scale_m%02d.nc"
        % (os.getenv("SCRATCH"), month),
    )
