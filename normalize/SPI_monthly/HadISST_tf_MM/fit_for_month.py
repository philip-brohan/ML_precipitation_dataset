...
parser.add_argument(
    ...
    default="%s/MLP/normalization/SPI_monthly/HadISST_tf_MM/v1" % os.getenv("SCRATCH"),
)
...
variance = tf.zeros([1, 721, 1440, 1], dtype=tf.float32)
for batch in trainingData:
    month = int(batch[1].numpy()[0][5:7])
    if month == args.month:
...
iris.save(
    shape,
    "%s/shape_m%02d.nc" % (args.opdir, args.month),
)
...
iris.save(
    location,
    "%s/location_m%02d.nc" % (args.opdir, args.month),
)
...
iris.save(
    scale,
    "%s/scale_m%02d.nc" % (args.opdir, args.month),
)
