...
print("source.shape, source.dtype:", getattr(source, "shape", None), getattr(source, "dtype", None))
print("target.shape, target.dtype:", getattr(target, "shape", None), getattr(target, "dtype", None))
print("source sample (first 3 rows):\n", source[:3])
print("target sample (first 10):\n", target[:10])
print("target stats: min, mean, max:", float(np.nanmin(target)), float(np.nanmean(target)), float(np.nanmax(target)))
