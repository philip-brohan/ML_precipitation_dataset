...
# Mean = 0.5025 here - because it only has data since 1979
...
def load_fitted(month):
    shape = iris.load_cube(
        "%s/MLP/normalisation/SPI_monthly/GPCP_tf_MM/blended/shape_m%02d.nc"
        % (os.getenv("SCRATCH"), month),
    )
    location = iris.load_cube(
        "%s/MLP/normalisation/SPI_monthly/GPCP_tf_MM/blended/location_m%02d.nc"
        % (os.getenv("SCRATCH"), month),
    )
    scale = iris.load_cube(
        "%s/MLP/normalisation/SPI_monthly/GPCP_tf_MM/blended/scale_m%02d.nc"
        % (os.getenv("SCRATCH"), month),
    )
...
def match_normal(raw, gamma_p, norm_mean=0.5025, norm_sd=0.2):
...
def match_original(normalised, gamma_p, norm_mean=0.5025, norm_sd=0.2):
...
def normalise_cube(raw, shape, location, scale, norm_mean=0.5025, norm_sd=0.2):
...
def unnormalise_cube(normalised, shape, location, scale, norm_mean=0.5025, norm_sd=0.2):
