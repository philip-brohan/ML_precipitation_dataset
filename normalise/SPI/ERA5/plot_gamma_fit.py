...
location = iris.load_cube("%s/MLP/normalisation/SPI/ERA5/location.nc" % os.getenv("SCRATCH"))
...
plots.plotFieldAxes(ax_shape,shape,plotCube=plots.plot_cube(),cMap = cmocean.cm.thermal)

ax_location = fig.add_axes([0.05, 0.34, 0.9, 0.31])
plots.plotFieldAxes(ax_location,location,plotCube=plots.plot_cube(),cMap = cmocean.cm.thermal)

ax_scale = fig.add_axes([0.05, 0.01, 0.9, 0.31])
plots.plotFieldAxes(ax_scale,scale,plotCube=plots.plot_cube(),cMap = cmocean.cm.thermal)
